...
//Code Here
const taxedPrices = prices.map(el => el * 1.07)
...
//Code Here
const sumPopulations = populations.reduce((acc, curr) => acc + curr)
...
//Code Here
const monsterCP = monstersInYourPocket.filter(el => el.CP > 200)
...
const totals = orders.map(el => (el.price * el.tax) + orders.price)
...
const bobsTotal = purchases.reduce((acc, cur) => {
  if (cur.owner === "Bob") {
